...
static struct ObjectInfo            kDevice3_ObjectList[]                = {
    // pancake: "Pancake Program" is the silent stream sink the Stage renders into. Like
    // "Pancake Mic" it exposes no controls; nobody monitors it, so it is inaudible to you.
#if kDevice3_HasInput
    { kObjectID_Stream_Input,           kObjectType_Stream,     kAudioObjectPropertyScopeInput  },
#endif
#if kDevice3_HasOutput
    { kObjectID_Stream_Output,          kObjectType_Stream,     kAudioObjectPropertyScopeOutput },
#endif
};
        ...
        case kObjectID_Device3:
            {
                if (scope == kAudioObjectPropertyScopeGlobal)
                {
                    return kDevice3_ObjectListSize;
                }

                UInt32 count = 0;
                for (UInt32 i = 0; i < kDevice3_ObjectListSize; i++)
                {
                    count += (kDevice3_ObjectList[i].scope == scope);
                }
...
static OSStatus	Pancake_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioPlugInPropertyDeviceList:
			...
			else if(theNumberItemsToFetch > 1)
			{
				((AudioObjectID*)outData)[0] = kObjectID_Device;
                ((AudioObjectID*)outData)[1] = kObjectID_Device2;
			}
		...
		case kAudioBoxPropertyDeviceList:
			...
			if(gBox_Acquired)
			{
                ...
                else
                {
                    ...
                    else if (inDataSize >= sizeof(AudioObjectID) * 2)
                    {
                        ((AudioObjectID*)outData)[0] = kObjectID_Device;
                        ((AudioObjectID*)outData)[1] = kObjectID_Device2;
		...
		case kAudioObjectPropertyOwnedObjects:
            ...
            switch (inObjectID) {
                ...
                case kObjectID_Device3:
                    for (UInt32 i = 0, k = 0; k < theNumberItemsToFetch; i++)
                    {
                        if (kDevice3_ObjectList[i].scope == inAddress->mScope || inAddress->mScope == kAudioObjectPropertyScopeGlobal)
                        {
		...
		case kAudioDevicePropertyStreams:
            ...
            switch (inObjectID) {
                ...
                case kObjectID_Device3:
                    for (UInt32 i = 0, k = 0; k < theNumberItemsToFetch; i++)
                    {
                        if ((kDevice3_ObjectList[i].type == kObjectType_Stream) &&
                            (kDevice3_ObjectList[i].scope == inAddress->mScope || inAddress->mScope == kAudioObjectPropertyScopeGlobal))
                        {
                            ((AudioObjectID*)outData)[k++] = kDevice3_ObjectList[i].id;
                        }
                    }
